...
public class FreeReasonsServiceImpl implements FreeReasonsService {
    ...
    public JSONObject selectResultByConditions(Map<String, String> reqmap) {
        JSONObject result = supperSearchService.supperSearch(new FreeReasonsTb(),reqmap);
       /* String str = "{\"total\":12,\"page\":1,\"rows\":[]}";
        JSONObject result = JSONObject.parseObject(str);


        int count =0;
        List<FreeReasonsTb> list =null;
        List<Map<String, Object>> resList =new ArrayList<>();
        Map searchMap = supperSearchService.getBaseSearch(new FreeReasonsTb(),reqmap);
        logger.info(searchMap);
        if(searchMap!=null&&!searchMap.isEmpty()){
            FreeReasonsTb baseQuery =(FreeReasonsTb)searchMap.get("base");
            ...
            if(count>0){
                ...
                if (list != null && !list.isEmpty()) {
                    for (FreeReasonsTb product : list) {
                        OrmUtil<FreeReasonsTb> otm = new OrmUtil<>();
