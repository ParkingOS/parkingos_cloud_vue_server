...
public class BindCarTypeServiceImpl implements BindCarTypeService {
    ...
    public JSONObject selectResultByConditions(Map<String, String> reqmap) {
        JSONObject result = supperSearchService.supperSearch(new CarNumberTypeTb(),reqmap);
        /*String str = "{\"total\":12,\"page\":1,\"rows\":[]}";
        JSONObject result = JSONObject.parseObject(str);


        int count =0;
        List<CarNumberTypeTb> list =null;
        List<Map<String, Object>> resList =new ArrayList<>();
        Map searchMap = supperSearchService.getBaseSearch(new CarNumberTypeTb(),reqmap);
        logger.info(searchMap);
        if(searchMap!=null&&!searchMap.isEmpty()){
            CarNumberTypeTb baseQuery =(CarNumberTypeTb)searchMap.get("base");
            ...
            if(count>0){
                ...
                if (list != null && !list.isEmpty()) {
                    for (CarNumberTypeTb product : list) {
                        OrmUtil<CarNumberTypeTb> otm = new OrmUtil<>();
