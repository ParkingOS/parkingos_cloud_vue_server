            ...
            if(fixCodeTb.getType()==1){//减免券
                ...
                if(ticketUnit==4){//减免金额
                    if(amount>fixCodeTb.getMoneyLimit()){
                        ...
                        Map<String,Object> retMap = ticketService.createTicket(shopid,amount,5,0,1,0);
                    ...
                    if(amount>fixCodeTb.getTimeLimit()){
                        ...
                        Map<String,Object> retMap = ticketService.createTicket(shopid,amount,3,0,1,0);
                        if(retMap.get("result") == -1 || retMap.get("result") == -2){
                            logger.info("生成减免劵出错，用劵失败");
                            result.put("state",0);
                            result.put("error",retMap.get("error"));
                ...
                if(fixCodeTb.getFreeLimit()<1){
                    ...
                    Map<String,Object> retMap = ticketService.createTicket(shopid,0,4,0,1,0);
                    if(retMap.get("result") == -1 || retMap.get("result") == -2){
                        logger.info("生成减免劵出错，用劵失败");
                        result.put("state",0);
                        result.put("error",retMap.get("error"));
